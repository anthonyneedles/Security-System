...
/****************************************************************************************
* GetKey() - This function provides public access to keyBuffer. It clears keyBuffer
...
void KeyInit(void){
    ...
    /* Key port init */
    SIM_SCGC5 |= SIM_SCGC5_PORTC(1);     //Make sure that the gated clock for PORTC is on
    PORTC_PCR3=PORT_PCR_MUX(1)|PORT_PCR_PS_MASK|PORT_PCR_PE_MASK;   //Columns
    ...
    PORTC_PCR7=PORT_PCR_MUX(1);     //Rows
...
*
*             ANTHONY NEEDLES - Added timeslice counter so that this task occurs
*             every [2*SLICE_PERIOD] = 20ms.
* (Public)
****************************************************************************************/
void KeyTask(void) {
    ...
    static INT8U kt_slice_counter = 1;

    DB2_TURN_ON();
    if(kt_slice_counter > 0){
        kt_slice_counter = 0;
        cur_key = keyScan();
